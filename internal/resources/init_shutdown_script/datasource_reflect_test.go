...
// TestInitShutdownScriptDataSourceModel_MatchesSchema verifies that every
// tfsdk tag on InitShutdownScriptDataSourceModel has a corresponding
...
func TestInitShutdownScriptDataSourceModel_MatchesSchema(t *testing.T) {
	d := &InitShutdownScriptDataSource{}
	...
	modelType := reflect.TypeOf(InitShutdownScriptDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("InitShutdownScriptDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
