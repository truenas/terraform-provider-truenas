...
// TestFilesystemPermissionsDataSourceModel_MatchesSchema verifies that
// every tfsdk tag on FilesystemPermissionsDataSourceModel has a
// corresponding attribute in the datasource schema, and vice versa.
...
func TestFilesystemPermissionsDataSourceModel_MatchesSchema(t *testing.T) {
	d := &FilesystemPermissionsDataSource{}
	...
	modelType := reflect.TypeOf(FilesystemPermissionsDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("FilesystemPermissionsDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
