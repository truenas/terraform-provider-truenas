...
// TestKeychainSSHConnectionDataSourceModel_MatchesSchema verifies that
// every tfsdk tag on KeychainSSHConnectionDataSourceModel has a
...
func TestKeychainSSHConnectionDataSourceModel_MatchesSchema(t *testing.T) {
	d := &KeychainSSHConnectionDataSource{}
	...
	modelType := reflect.TypeOf(KeychainSSHConnectionDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("KeychainSSHConnectionDataSourceModel tfsdk tags %v do not match datasource schema attributes %v",
			modelFields, schemaAttrs)
	}
}
