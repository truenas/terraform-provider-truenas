...
// TestDockerNetworkDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on DockerNetworkDataSourceModel has a corresponding attribute in the
...
func TestDockerNetworkDataSourceModel_MatchesSchema(t *testing.T) {
	d := &DockerNetworkDataSource{}
	...
	modelType := reflect.TypeOf(DockerNetworkDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("DockerNetworkDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
