...
// TestReportingExporterDataSourceModel_MatchesSchema verifies that every
// tfsdk tag on ReportingExporterDataSourceModel has a corresponding
...
func TestReportingExporterDataSourceModel_MatchesSchema(t *testing.T) {
	d := &ReportingExporterDataSource{}
	...
	modelType := reflect.TypeOf(ReportingExporterDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("ReportingExporterDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
