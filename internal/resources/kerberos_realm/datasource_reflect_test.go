...
// TestKerberosRealmDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on KerberosRealmDataSourceModel has a corresponding attribute in the
...
func TestKerberosRealmDataSourceModel_MatchesSchema(t *testing.T) {
	d := &KerberosRealmDataSource{}
	...
	modelType := reflect.TypeOf(KerberosRealmDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("KerberosRealmDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
