...
// TestPrivilegeDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on PrivilegeDataSourceModel has a corresponding attribute in the
...
func TestPrivilegeDataSourceModel_MatchesSchema(t *testing.T) {
	d := &PrivilegeDataSource{}
	...
	modelType := reflect.TypeOf(PrivilegeDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("PrivilegeDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
