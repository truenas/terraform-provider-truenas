...
// TestVMwareDataSourceModel_MatchesSchema verifies that every tfsdk tag on
// VMwareDataSourceModel has a corresponding attribute in the datasource
...
func TestVMwareDataSourceModel_MatchesSchema(t *testing.T) {
	d := &VMwareDataSource{}
	...
	modelType := reflect.TypeOf(VMwareDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("VMwareDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
