...
// TestTwoFactorAuthDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on TwoFactorAuthDataSourceModel has a corresponding attribute in the
...
func TestTwoFactorAuthDataSourceModel_MatchesSchema(t *testing.T) {
	d := &TwoFactorAuthDataSource{}
	...
	modelType := reflect.TypeOf(TwoFactorAuthDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("TwoFactorAuthDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
