...
// TestCertificateDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on CertificateDataSourceModel has a corresponding attribute in the
...
func TestCertificateDataSourceModel_MatchesSchema(t *testing.T) {
	...
	modelType := reflect.TypeOf(CertificateDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("CertificateDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
