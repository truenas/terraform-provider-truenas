...
// TestRsyncTaskDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on RsyncTaskDataSourceModel has a corresponding attribute in the
...
func TestRsyncTaskDataSourceModel_MatchesSchema(t *testing.T) {
	d := &RsyncTaskDataSource{}
	...
	modelType := reflect.TypeOf(RsyncTaskDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("RsyncTaskDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
