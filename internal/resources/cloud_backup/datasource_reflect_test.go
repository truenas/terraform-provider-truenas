...
// TestCloudBackupDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on CloudBackupDataSourceModel has a corresponding attribute in the
...
func TestCloudBackupDataSourceModel_MatchesSchema(t *testing.T) {
	d := &CloudBackupDataSource{}
	...
	modelType := reflect.TypeOf(CloudBackupDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("CloudBackupDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
