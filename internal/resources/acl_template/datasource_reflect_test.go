...
// TestAclTemplateDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on AclTemplateDataSourceModel has a corresponding attribute in the
...
func TestAclTemplateDataSourceModel_MatchesSchema(t *testing.T) {
	d := &AclTemplateDataSource{}
	...
	modelType := reflect.TypeOf(AclTemplateDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("AclTemplateDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
