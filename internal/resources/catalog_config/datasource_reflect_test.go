...
// TestCatalogConfigDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on CatalogConfigDataSourceModel has a corresponding attribute in the
...
func TestCatalogConfigDataSourceModel_MatchesSchema(t *testing.T) {
	d := &CatalogConfigDataSource{}
	...
	modelType := reflect.TypeOf(CatalogConfigDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("CatalogConfigDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
