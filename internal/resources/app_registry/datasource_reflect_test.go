...
// TestAppRegistryDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on AppRegistryDataSourceModel has a corresponding attribute in the
...
func TestAppRegistryDataSourceModel_MatchesSchema(t *testing.T) {
	d := &AppRegistryDataSource{}
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("AppRegistryDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
