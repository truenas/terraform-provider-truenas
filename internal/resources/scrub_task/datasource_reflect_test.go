...
// TestScrubTaskDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on ScrubTaskDataSourceModel has a corresponding attribute in the
...
func TestScrubTaskDataSourceModel_MatchesSchema(t *testing.T) {
	d := &ScrubTaskDataSource{}
	...
	modelType := reflect.TypeOf(ScrubTaskDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("ScrubTaskDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
