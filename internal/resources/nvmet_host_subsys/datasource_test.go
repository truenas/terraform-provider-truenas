...
// TestHostSubsysDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on HostSubsysDataSourceModel has a corresponding attribute in the
...
func TestHostSubsysDataSourceModel_MatchesSchema(t *testing.T) {
	d := &HostSubsysDataSource{}
	...
	modelType := reflect.TypeOf(HostSubsysDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("HostSubsysDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
