...
// TestKerberosKeytabDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on KerberosKeytabDataSourceModel has a corresponding attribute in the
...
func TestKerberosKeytabDataSourceModel_MatchesSchema(t *testing.T) {
	d := &KerberosKeytabDataSource{}
	...
	modelType := reflect.TypeOf(KerberosKeytabDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("KerberosKeytabDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
