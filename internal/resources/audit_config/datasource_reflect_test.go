...
// TestAuditConfigDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on AuditConfigDataSourceModel has a corresponding attribute in the
...
func TestAuditConfigDataSourceModel_MatchesSchema(t *testing.T) {
	d := &AuditConfigDataSource{}
	...
	modelType := reflect.TypeOf(AuditConfigDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("AuditConfigDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
