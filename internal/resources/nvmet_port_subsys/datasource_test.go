...
// TestPortSubsysDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on PortSubsysDataSourceModel has a corresponding attribute in the
...
func TestPortSubsysDataSourceModel_MatchesSchema(t *testing.T) {
	d := &PortSubsysDataSource{}
	...
	modelType := reflect.TypeOf(PortSubsysDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("PortSubsysDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
