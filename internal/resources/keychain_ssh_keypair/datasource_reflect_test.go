...
// TestKeychainSSHKeyPairDataSourceModel_MatchesSchema verifies that every
// tfsdk tag on KeychainSSHKeyPairDataSourceModel has a corresponding
...
func TestKeychainSSHKeyPairDataSourceModel_MatchesSchema(t *testing.T) {
	d := &KeychainSSHKeyPairDataSource{}
	...
	modelType := reflect.TypeOf(KeychainSSHKeyPairDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("KeychainSSHKeyPairDataSourceModel tfsdk tags %v do not match datasource schema attributes %v",
