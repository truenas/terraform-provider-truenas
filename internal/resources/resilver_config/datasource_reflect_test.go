...
// TestResilverConfigDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on ResilverConfigDataSourceModel has a corresponding attribute in the
...
func TestResilverConfigDataSourceModel_MatchesSchema(t *testing.T) {
	d := &ResilverConfigDataSource{}
	...
	modelType := reflect.TypeOf(ResilverConfigDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("ResilverConfigDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
