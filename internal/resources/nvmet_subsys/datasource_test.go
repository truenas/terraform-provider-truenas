...
// TestNVMetSubsysDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on NVMetSubsysDataSourceModel has a corresponding attribute in the
...
func TestNVMetSubsysDataSourceModel_MatchesSchema(t *testing.T) {
	d := &NVMetSubsysDataSource{}
	...
	modelType := reflect.TypeOf(NVMetSubsysDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("NVMetSubsysDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
