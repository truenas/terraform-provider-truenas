...
// TestNVMetPortDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on NVMetPortDataSourceModel has a corresponding attribute in the
...
func TestNVMetPortDataSourceModel_MatchesSchema(t *testing.T) {
	d := &NVMetPortDataSource{}
	...
	modelType := reflect.TypeOf(NVMetPortDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("NVMetPortDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
