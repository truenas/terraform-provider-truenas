...
// TestNVMetNamespaceDataSourceModel_MatchesSchema verifies that every tfsdk
// tag on NVMetNamespaceDataSourceModel has a corresponding attribute in the
...
func TestNVMetNamespaceDataSourceModel_MatchesSchema(t *testing.T) {
	d := &NVMetNamespaceDataSource{}
	...
	modelType := reflect.TypeOf(NVMetNamespaceDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("NVMetNamespaceDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
