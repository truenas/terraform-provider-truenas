...
// TestLXCConfigDataSourceModel_MatchesSchema verifies that every tfsdk tag
// on LXCConfigDataSourceModel has a corresponding attribute in the
...
func TestLXCConfigDataSourceModel_MatchesSchema(t *testing.T) {
	d := &LXCConfigDataSource{}
	...
	modelType := reflect.TypeOf(LXCConfigDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("LXCConfigDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
