...
// TestCronjobDataSourceModel_MatchesSchema verifies that every tfsdk tag on
// CronjobDataSourceModel has a corresponding attribute in the datasource
...
func TestCronjobDataSourceModel_MatchesSchema(t *testing.T) {
	d := &CronjobDataSource{}
	...
	modelType := reflect.TypeOf(CronjobDataSourceModel{})
	...
	if !reflect.DeepEqual(schemaAttrs, modelFields) {
		t.Fatalf("CronjobDataSourceModel tfsdk tags %v do not match datasource schema attributes %v", modelFields, schemaAttrs)
	}
}
